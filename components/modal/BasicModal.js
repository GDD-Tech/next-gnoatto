      ...
      <Box sx={style}>
        ...
        {['none', 'right'].includes(direction) && 
          <div style={{ marginTop: '8px'}}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
              {eixoList.map((eixo) => (
                <Button key={eixo.id} variant="contained" color="error" onClick={() => handleAxles('right', eixo.value)} >
                  {eixo.value}
                </Button>
              ))}
            </div>
        ...
        {['none', 'left'].includes(direction) && 
          <div style={{ marginTop: '8px'}}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
              {eixoList.map((eixo) => (
                <Button key={eixo.id} variant="contained" color="success" onClick={() => handleAxles('left', eixo.value)} >
                  {eixo.value}
                </Button>
              ))}
            </div>
